                                 ...
                                 Bool_t isMassvsRadiusPlots = 0)
{
  ...
  TDirectoryFile *dirEvt = (TDirectoryFile *)dir->Get("histos_event");
  if (!dirEvt)
  {
    printf("\nEVENTS DIRECTORY NOT FOUND!\n");
    return;
  }
  TDirectoryFile *dirCasc = (TDirectoryFile *)dir->Get("histos_Casc");
  if (!dirCasc)
  {
    printf("\nCASC DIRECTORY NOT FOUND!\n");
    return;
  }
  TDirectoryFile *dirV0 = (TDirectoryFile *)dir->Get("histos_V0");
  if (!dirV0)
  {
  ...
  Float_t WidthPartLimits[2][numPart] = {{0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001}, {0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005}};

  // histo of generated particles (MC)
  TCanvas *canvasMC[numPart];
  TCanvas *canvasMC_Rdependence[numPart];
  TH3F *fHistGen3DV0s = {nullptr};
  TH3F *fHistGen3DCasc = {nullptr};
  TH2F *fHistGen2DV0s_Rintegrated = {nullptr};
  TH2F *fHistGen2DCasc_Rintegrated = {nullptr};
  TH2F *fHistGen2DV0s_ptintegrated = {nullptr};
  TH2F *fHistGen2DCasc_ptintegrated = {nullptr};
  ...
  if (isMC)
  {
    fHistGen3DV0s = (TH3F *)dirV0->Get("GeneratedV0s");
    if (!fHistGen3DV0s)
    {
      cout << "GeneratedV0s histo not found! " << endl;
      return;
    }
    fHistGen3DCasc = (TH3F *)dirCasc->Get("GeneratedCascades");
    if (!fHistGen3DCasc)
    {
      cout << "GeneratedCascades histo not found! " << endl;
      return;
    }
  }
  ...
  {
    // Radius integrated
    fHistGen2DV0s_Rintegrated = (TH2F *)fHistGen3DV0s->Project3D("yxo");
    fHistGen2DV0s_Rintegrated->SetName("GeneratedParticles2DV0s_RadiusIntegrated");
    fHistGen2DV0s_Rintegrated->Write();
    fHistGen2DCasc_Rintegrated = (TH2F *)fHistGen3DCasc->Project3D("yxo");
    fHistGen2DCasc_Rintegrated->SetName("GeneratedParticles2DCasc_RadiusIntegrated");
    fHistGen2DCasc_Rintegrated->Write();

    // pt integrated
    fHistGen2DV0s_ptintegrated = (TH2F *)fHistGen3DV0s->Project3D("zxo");
    fHistGen2DV0s_ptintegrated->SetName("GeneratedParticles2DV0s_ptIntegrated");
    fHistGen2DV0s_ptintegrated->Write();
    fHistGen2DCasc_ptintegrated = (TH2F *)fHistGen3DCasc->Project3D("zxo");
    fHistGen2DCasc_ptintegrated->SetName("GeneratedParticles2DCasc_ptIntegrated");
    fHistGen2DCasc_ptintegrated->Write();
  }
  ...
  TString TopVarV0Input[NTopV0variables] = {"DCAPosToPV", "DCANegToPV", "DCAV0Daughters", "CosPA", "Radius"};
  ...
  TString TopVarCascInput[NTopCascInput] = {"CascPt", "V0Lifetime", "CascCosPA", "V0CosPA", "V0CosPAToXi", "CascRadius", "V0Radius", "InvMassLambda", "DcaCascDaughters", "DcaV0Daughters", "DcaBachToPV", "DcaV0ToPV", "DcaPosToPV", "DcaNegToPV", "CascRapidityXi", "CascLifetimeXi", "CascRapidityOmega", "CascLifetimeOmega"};
  ...
  Float_t NPt[num][2] = {{0., 6.}, {0., 0.7}, {0.7, 1.}, {1., 1.5}, {1.5, 2}, {2., 2.5}, {2.5, 3.}, {3., 3.5}, {3.5, 4.}, {4., 6.}};
  TString SPt[num] = {"0.0-6.0", "0.0-0.7", "0.7-1.0", "1.0-1.5", "1.5-2.0", "2.0-2.5", "2.5-3.0", "3.0-3.5", "3.5-4.0", "4.0-6.0"};
  Float_t PtVector[num] = {0., 0.7, 1., 1.5, 2., 2.5, 3., 3.5, 4., 6.};
  ...
  {
    fhistodEdxDaughters2D[iDaughter] = (TH2F *)dirV0->Get("TPCPID" + LambdaDaugtersLong[iDaughter] + "FromLambda");
  ...
  for (Int_t part = 0; part < numPart; part++)
  {
    ...
    if (isMC)
    {
      ...
      if (part < 3)
      { // V0s
        fHistGen_Rintegrated[part] = (TH1F *)fHistGen2DV0s_Rintegrated->ProjectionY("GeneratedParticles_Rintegrated_" + NamePart[part], fHistGen2DV0s_Rintegrated->GetXaxis()->FindBin(partGen + 0.5), fHistGen2DV0s_Rintegrated->GetXaxis()->FindBin(partGen + 0.5), "e");
        fHistGen_ptintegrated[part] = (TH1F *)fHistGen2DV0s_ptintegrated->ProjectionY("GeneratedParticles_ptintegrated_" + NamePart[part], fHistGen2DV0s_ptintegrated->GetXaxis()->FindBin(partGen + 0.5), fHistGen2DV0s_ptintegrated->GetXaxis()->FindBin(partGen + 0.5), "e");
        fhistoInvMass3DTrue[part] = (TH3F *)dirV0->Get(NamehistoInvMass[part] + "True");
      }
      else
      {
        fHistGen_Rintegrated[part] = (TH1F *)fHistGen2DCasc_Rintegrated->ProjectionY("GeneratedParticles_Rintegrated_" + NamePart[part], fHistGen2DCasc_Rintegrated->GetXaxis()->FindBin(partGen + 0.5), fHistGen2DCasc_Rintegrated->GetXaxis()->FindBin(partGen + 0.5), "e");
        fHistGen_ptintegrated[part] = (TH1F *)fHistGen2DCasc_ptintegrated->ProjectionY("GeneratedParticles_ptintegrated_" + NamePart[part], fHistGen2DCasc_ptintegrated->GetXaxis()->FindBin(partGen + 0.5), fHistGen2DCasc_ptintegrated->GetXaxis()->FindBin(partGen + 0.5), "e");
    ...
    if (part < 3)
    {
      cout << NamehistoInvMass[part] << NameHisto << endl;
    ...
    if (!fhistoInvMass3D_etalabel[part])
    {
  ...
  if (isMC)
  {
    histoPartPerEvent->GetYaxis()->SetRangeUser(0, 1);
