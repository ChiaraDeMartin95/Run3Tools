...
void QAplots(string period = "LHC25ai")
{
  ...
  char pass_name[2][30] = {"", "skimmed"};
  //char pass_name[2][30] = {"apass1", "apass1"};
  const int nruns = 7;
  ...
  //int runnumber[nruns] = {556639, 556640, 556641, 556662, 556664, 556680, 556716, 556734, 556741, 556767, 556816, 556834, 556872, 556889, 556907, 556909, 556913, 556923, 556939, 556954, 556958, 556979, 556981, 556997, 557012, 557019, 557021, 557026, 557074, 557104, 557112, 557118, 557119, 557138, 557149, 557226, 557233, 557251, 557271, 557291, 557299, 557321, 557336, 557339, 557350, 557374}; // batch3,4
  int runnumber[nruns] = {566017, 566029, 566030, 566097, 566109, 566122, 566123};
  //------------ read files
  ...
  {
    if (gSystem->GetPathInfo(Form("../TriggerForRun3/EventFiltering2025/AnalysisResults/LHC25ai/AnalysisResults_fullrun_batch1_%s_%d.root", period.c_str(),runnumber[irun]), dummy1, dummy2, dummy3, dummy4) != 0)
      cout << "File Not Found! Try again" << endl;
    file_in[irun] = new TFile(Form("../TriggerForRun3/EventFiltering2025/AnalysisResults/LHC25ai/AnalysisResults_fullrun_batch1_%s_%d.root", period.c_str(), runnumber[irun]), "read");
    printf("Open File: %s\n", file_in[irun]->GetName());

    if (gSystem->GetPathInfo(Form("../TriggerForRun3/EventFiltering2025_skimmedDATA/AnalysisResults_fullrun_%s_batch1_%s_%d.root", pass_name[1], period.c_str(), runnumber[irun]), dummy1, dummy2, dummy3, dummy4) != 0)
      cout << "File Not Found! Try again" << endl;
    file_in_skimmed[irun] = new TFile(Form("../TriggerForRun3/EventFiltering2025_skimmedDATA/AnalysisResults_fullrun_%s_batch1_%s_%d.root", pass_name[1], period.c_str(), runnumber[irun]), "read");
  ...
  for (int irun = 0; irun < nruns; irun++)
  {
    ...
    for (int ibin = 0; ibin < hEvSel_skimmed_filters[irun]->GetNbinsX(); ibin++)
      hRatio[irun]->SetBinContent(ibin + 1, hEvSel_skimmed_filters[irun]->GetBinContent(ibin + 1) / hEvSel_filters[irun]->GetBinContent(ibin + 1));
    // hRatio[irun]->Sumw2();
  }
  ...
  legratio->Draw("same");
  cratio->SaveAs("ratioskimmunskimm_LHC25ai_batch1.png");
  ...
  for (int i = 0; i < nruns; i++)
  {
    color = nruns - i - 1 + FI;
    for (Int_t j = 1; j <= hEvSelFinal[i]->GetNbinsX(); j++)
      hEvSelFinal[i]->GetXaxis()->SetBinLabel(j, h->GetXaxis()->GetBinLabel(j));
  ...
  for (int ifilter = 0; ifilter < 7; ifilter++)
  {
    ...
    if (ifilter == 0)
      cselectivity[ifilter]->SaveAs(Form("cselectivity_period_%s_batch1_filter_Omega.png", period.c_str()));
    if (ifilter == 1)
      cselectivity[ifilter]->SaveAs(Form("cselectivity_period_%s_batch1_filter_hOmega.png", period.c_str()));
    if (ifilter == 2)
      cselectivity[ifilter]->SaveAs(Form("cselectivity_period_%s_batch1_filter_Xi-N.png", period.c_str()));
    if (ifilter == 3)
      cselectivity[ifilter]->SaveAs(Form("cselectivity_period_%s_batch1_filter_HighMult+Omega.png", period.c_str()));
    if (ifilter == 4)
      cselectivity[ifilter]->SaveAs(Form("cselectivity_period_%s_batch1_filter_Tracked Omega.png", period.c_str()));
    if (ifilter == 5)
      cselectivity[ifilter]->SaveAs(Form("cselectivity_period_%s_batch1_filter_Double Omega.png", period.c_str()));
    if (ifilter == 6)
      cselectivity[ifilter]->SaveAs(Form("cselectivity_period_%s_batch1_filter_Omega+Xi.png", period.c_str()));
  }
  ...
  hselectivity_period->SetTitle(Form("%s batch1", period.c_str()));
  ...
  // cselectivity_period->SaveAs(Form("cselectivity_period_%s_batch1.png",period.c_str()));
}
